...
colnames(accuracy.table) <- c("Variables used in the model", "Accuracy", "Accuracy with healthy eyes")
pander(accuracy.table, justify = c('left', 'right', 'right'))
```
...
classification <- classify(data, vars=varRims)
pander(classification$performance.glaucoma$table)
```

### Performance without healthy eyes

```{r performance-without-healthy-eyes}
pander(classification$performance.glaucoma$byClass)
```
...
pander(classification$performance$byClass)
```

## Model using sectors G and TI
